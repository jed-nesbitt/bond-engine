...
def main() -> None:
    ...
    parser.add_argument("--input", type=str, default="example_bonds.csv", help="Path to input CSV")
    ...
    out_csv = outdir / "bond_statistics.csv"
    out_xlsx = outdir / "bond_statistics.xlsx"
    ...
    if len(ok) == 0:
        ...
        top_risk = (
            ...
            .loc[:, ["bond_id", "quantity", "mv_dirty", "ytm", "modified_duration_years", "dv01_total", "dirty_price", "clean_price"]]
            .head(10)
        )
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        def auto_width(ws, min_w: int = 10, max_w: int = 45) -> None:
            # simple width heuristic: max string length in each column
        ...
        def apply_formats_by_headers(ws, header_row: int, end_row: int) -> None:
            # Map header -> Excel number format
            date_cols = {"issue_date", "settle_date", "maturity_date"}
            pct_cols = {"coupon_rate", "ytm", "input_ytm", "weighted_ytm"}
            price_cols_4dp = {
                "clean_price", "dirty_price", "accrued_interest",
                "price_change_+25bp", "price_change_+100bp",
            }
            money_cols_2dp = {
                "mv_dirty", "mv_clean", "accrued_interest_total",
                "Total_MV_Dirty", "Total_MV_Clean", "Total_Accrued_Interest",
            }
            dv01_cols = {"dv01_per_100", "dv01_total", "total_dv01", "Total_DV01"}
            dur_cols = {
                "macaulay_duration_years", "modified_duration_years",
                "weighted_mod_dur", "weighted_mod_dur_years",
                "Weighted_Modified_Duration_Years", "Weighted_Macaulay_Duration_Years",
                "time_to_maturity_years", "Weighted_TTM_Years",
            }
            ...
            for h, c in headers.items():
                ...
                for r_ in range(header_row + 1, end_row + 1):
                    ws.cell(row=r_, column=c).number_format = fmt

        def format_table_at(ws, header_row: int, start_col: int = 1) -> None:
            # Determine end_col from header row
            end_col = start_col
            ...
            # Determine end_row until blank row (within the table width)
            end_row = header_row
            r_ = header_row + 1
            while r_ <= ws.max_row:
                row_blank = True
                for c in range(start_col, end_col + 1):
                    if not is_blank(ws.cell(row=r_, column=c).value):
                        row_blank = False
                        break
                if row_blank:
                    break
        ...
        # Main summary block starts at row 1
        format_table_at(ws_sum, header_row=1)

        # Titles + subsequent tables (look for the marker rows you wrote)
        for title in ("Breakdown_by_DayCount", "Breakdown_by_Frequency", "Top_10_by_Absolute_DV01"):
            ...
            if title_row is None:
                continue
            ws_sum.cell(row=title_row, column=1).font = title_font  # make title bold
